...
// Sliding panel content per visual step
const PANEL_SLIDES = [
  {
    icon: (
...
function RegisterTrader() {
  ...
  const handleChange = (e) => {
    ...
    setForm({
      ...form,
      [name]: noCapFields.includes(name) ? value : value.toUpperCase(),
    });
  };
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${API}/payment/register-payment`, paymentData);
      if (res.data.success) {
  ...
  // VALIDATE STEP — untouched (internal steps 1-5)
  ...
  const handleSubmit = async (e) => {
    ...
    Object.keys(form).forEach((key) => {
      if (key === "profile_image" && form[key]) {
        ...
        formData.append("phone", form[key].replace(/\s/g, ""));
    ...
    try {
      const res = await axios.post(`${API}/users/register-trader`, formData, {
  ...
  // Password strength — untouched logic
  ...
  return (
    <>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
            ...
            <div className="p-6">
              ...
              <div className="space-y-3">
                ...
                <button
                  onClick={() => { setShowSuccessModal(false); localStorage.setItem("user_code", registrationData?.user_code); navigate("/login", { state: { user_code: registrationData?.user_code } }); }}
      ...
      {/* ERROR MODAL — untouched */}
      {showErrorModal && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-[300] p-4 animate-fade-in">
            ...
            <div className="px-5 py-4">
              ...
              <div className="space-y-2">
                {(errorMessage.includes("already registered") || errorMessage.includes("already in use")) && (
                  <button onClick={() => { setShowErrorModal(false); navigate("/login"); }} className="w-full bg-blue-500 text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-blue-600 transition-all">
                    Go to Login
                  </button>
                )}
                <button onClick={() => { setShowErrorModal(false); setErrorMessage(""); }} className="w-full bg-gray-100 text-gray-600 py-2.5 rounded-xl text-sm font-semibold hover:bg-gray-200 transition-all">
                  {t.try_again}
                </button>
              </div>
            </div>
      ...
      {showPreviewModal && (
        ...
        >
          <div
            className="bg-white rounded-t-[4px] md:rounded-[4px] w-full md:max-w-lg overflow-hidden"
          ...
          >
            ...
            <div className="p-5 max-h-[65vh] overflow-y-auto space-y-3">
              ...
              <div className="border border-gray-100 rounded-[4px] overflow-hidden">
                ...
                <div className="px-4 py-3 text-sm">
                  <span className="text-gray-500">Password:</span> <span className="text-green-600 font-medium">✓ {t.set}</span>
                </div>
              </div>
            ...
            <div className="px-5 py-4 border-t border-gray-100 flex gap-3">
              <button
                type="button"
                onClick={() => setShowPreviewModal(false)}
                className="flex-1 py-2.5 rounded-[4px] border border-gray-200 text-gray-600 text-sm font-semibold hover:bg-gray-50 transition-all"
              >
                Edit details
              </button>
              <button
                ...
                className="flex-1 py-2.5 rounded-[4px] bg-[#1a3a8f] text-[#F5C518] text-sm font-semibold hover:bg-[#0f2460] transition-all disabled:opacity-50"
              >
          ...
          <div className="hidden md:flex flex-col w-full md:w-5/12 bg-white border border-gray-200 rounded-[4px] overflow-hidden">
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 overflow-hidden min-h-[220px]">
              <div
                ...
                style={{
                  animation: `${slideDir === "left" ? "slideInFromRight" : "slideInFromLeft"} 0.35s ease forwards`,
                }}
              >
            ...
            <div className="flex justify-center gap-2 py-4 border-t border-gray-100">
              {[1, 2, 3].map((n) => (
                <div
                  key={n}
                  className={`h-1.5 rounded-full transition-all duration-300 ${currentStep === n ? "w-6 bg-[#1a3a8f]" : "w-1.5 bg-gray-200"}`}
                />
              ))}
            </div>
          </div>
          ...
          <div className="w-full md:w-7/12 bg-white border border-gray-200 rounded-[4px] overflow-hidden">
              ...
              <div className="flex items-center">
                {[1, 2, 3].map((step) => (
                  <div key={step} className="flex items-center flex-1 min-w-0">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0 border-2 transition-all ${
                      currentStep >= step ? "bg-[#1a3a8f] border-[#1a3a8f] text-[#F5C518]" : "bg-white border-gray-200 text-gray-400"
                    }`}>
                      {currentStep > step ? (
                    ...
                    {step < 3 && (
                      <div className={`flex-1 h-0.5 mx-1.5 min-w-0 transition-all ${currentStep > step ? "bg-[#1a3a8f]" : "bg-gray-200"}`} />
                    )}
                  </div>
                ))}
              </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); if (validateStep(5)) setShowPreviewModal(true); }} className="p-5 space-y-4">
                ...
                <>
                  <div>
                    ...
                    <div>
                      ...
                      />
                      {form.profile_image && (
                        <p className="text-xs text-green-600 mt-1">✓ {t.selected}: {form.profile_image.name}</p>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex gap-2 pt-2 border-t border-gray-100">
                {currentStep < 3 ? (
                  <button type="button" onClick={handleNextStep} className="flex-1 bg-[#1a3a8f] text-[#F5C518] py-2.5 rounded-[3px] text-sm font-semibold hover:bg-[#0f2460] transition-all">
                    {t.next}
                  </button>
                ) : (
                  <button type="submit" className="flex-1 bg-[#1a3a8f] text-[#F5C518] py-2.5 rounded-[3px] text-sm font-semibold hover:bg-[#0f2460] transition-all">
                    Preview & submit
                  </button>
                )}
                {currentStep > 1 && (
                  <button type="button" onClick={handlePreviousStep} className="flex-1 bg-white text-gray-600 py-2.5 rounded-[3px] text-sm font-semibold border border-gray-200 hover:bg-gray-50 transition-all">
                    {t.back}
                  </button>
                )}
                <button type="button" onClick={handleClear} className="flex-1 bg-white text-gray-500 py-2.5 rounded-[3px] text-sm font-semibold border border-gray-200 hover:bg-gray-50 transition-all">
                  {t.clear}
                </button>
                <button type="button" onClick={() => navigate("/")} className="flex-1 bg-white text-red-500 py-2.5 rounded-[3px] text-sm font-semibold border border-red-100 hover:bg-red-50 transition-all">
                  {t.exit}
                </button>
              </div>
            </form>
