export default function BuyOptionsModal({ product, onClose, onOrder, onContact }) {
  return (
    <div
      className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-end md:items-center justify-center"
      onClick={onClose}
    >
      <div
        className="bg-white rounded-t-3xl md:rounded-3xl shadow-2xl w-full max-w-sm pb-8"
        onClick={e => e.stopPropagation()}
      >
        {/* Drag handle */}
        <div className="flex justify-center pt-3 pb-1">
          <div className="w-10 h-1 bg-gray-200 rounded-full" />
        </div>

        {/* Header */}
        <div className="px-6 pt-3 pb-2 text-center">
          <h3 className="font-bold text-gray-800 text-base">How would you like to proceed?</h3>
          <p className="text-xs text-gray-400 mt-1">{product.name}</p>
        </div>

        <div className="px-5 pt-3 space-y-3">

          {/* Add Order */}
          <button
            onClick={onOrder}
            className="flex items-center gap-3 w-full px-4 py-3 rounded-2xl bg-blue-50 hover:bg-blue-100 transition border border-blue-100"
          >
            <span className="w-10 h-10 rounded-2xl bg-blue-600 flex items-center justify-center flex-shrink-0">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/>
                <path d="M1 1h4l2.68 13.39a2 2 0 001.99 1.61h9.72a2 2 0 001.99-1.61L23 6H6"/>
              </svg>
            </span>
            <div className="text-left">
              <p className="font-semibold text-blue-800 text-sm">Place an Order</p>
              <p className="text-xs text-blue-400">Fill order form & submit</p>
            </div>
          </button>

          {/* Contact Trader */}
          <button
            onClick={onContact}
            className="flex items-center gap-3 w-full px-4 py-3 rounded-2xl bg-green-50 hover:bg-green-100 transition border border-green-100"
          >
            <span className="w-10 h-10 rounded-2xl bg-[#25D366] flex items-center justify-center flex-shrink-0">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="#fff">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
              </svg>
            </span>
            <div className="text-left">
              <p className="font-semibold text-green-800 text-sm">Contact {product.trader_name || "Trader"}</p>
              <p className="text-xs text-green-500">Call, WhatsApp, SMS & more</p>
            </div>
          </button>

          {/* Cancel */}
          <button
            onClick={onClose}
            className="w-full py-3 rounded-2xl bg-green-500 hover:bg-green-600 text-white font-semibold text-sm transition mt-1"
          >
            Cancel
          </button>

        </div>
      </div>
    </div>
  );
}